/**
 *
 */

#ifndef __DebugProcessor_H__
#define __DebugProcessor_H__

//-----------------//
//   C++  StdLib   //
//-----------------//
#include <iostream>
#include <string>

//----------//
//   ROOT   //
//----------//
#include "TClonesArray.h"

//-----------//
//   hpstr   //
//-----------//
#include "Processor.h"
#include "BaseSelector.h"
#include "Track.h"
#include "Event.h"
#include "TrackHistos.h"
#include "EventHeader.h"

// Forward declarations
class TTree; 

class DebugProcessor : public Processor { 

    public: 

        /**
         * Class constructor. 
         *
         * @param name Name for this instance of the class.
         * @param process The Process class associated with Processor, provided
         *                by the processing framework.
         */
        DebugProcessor(const std::string& name, Process& process); 

        /** Destructor */
        ~DebugProcessor(); 

        /**
         * Configure the Ana Processor
         * @param parameters The configuration parameters
         */
        virtual void configure(const ParameterSet& parameters);

        /**
         * Process the event and put new data products into it.
         * @param event The Event to process.
         */
        virtual bool process(IEvent* ievent);

        /**
         * Callback for the Processor to take any necessary
         * action when the processing of events starts.
         */
        virtual void initialize(TTree* tree);

        /**
         * Callback for the Processor to take any necessary
         * action when the processing of events finishes.
         */
        virtual void finalize();

    private: 

        /** Container to hold all Track objects. */
        std::vector<Track*> * tracks_{};
        TBranch*           btracks_{nullptr};

        std::vector<Vertex*> * vtxs_{};
        TBranch*            bvtxs_{nullptr};

        EventHeader * evth_{nullptr};
        TBranch*           bevth_{nullptr};

        // Track Collection name
        std::string trkCollName_;

        std::string vtxCollName_;

        // Track Selector configuration
        std::string selectionCfg_;
        std::shared_ptr<BaseSelector> trkSelector_;
        std::vector<std::string> regionSelections_;
        int isData_ = 1;

        // Containers to hold histogrammer info
        std::string histCfgFilename_;
        std::string truthHistCfgFilename_;
        TrackHistos* trkHistos_{nullptr};
        TrackHistos* truthHistos_{nullptr};
        bool doTruth_{false};
        int debug_{0};
        



}; // DebugProcessor

#endif // __DebugProcessor_
